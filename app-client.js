...
// Decorative local photography: no announcements during automatic transitions.
const townPanel=$('mullingar'),townSlides=[...townPanel.querySelectorAll('.town-slide')];
let townIndex=0,townTimer=null,townPlaying=!reduced.matches,townVisible=false,townHover=false,townFocus=false;
function townShow(index){townIndex=(index+townSlides.length)%townSlides.length;townSlides.forEach((slide,i)=>slide.classList.toggle('active',i===townIndex));$('town-count').textContent=String(townIndex+1).padStart(2,'0')+' / 03';}
function townSchedule(){clearInterval(townTimer);townTimer=null;$('town-pause').textContent=townPlaying?townPanel.dataset.pause:townPanel.dataset.play;$('town-pause').setAttribute('aria-pressed',String(!townPlaying));if(townPlaying&&townVisible&&!townHover&&!townFocus&&!document.hidden&&!reduced.matches)townTimer=setInterval(()=>townShow(townIndex+1),6500);}
$('town-prev').addEventListener('click',()=>{townShow(townIndex-1);townSchedule();});$('town-next').addEventListener('click',()=>{townShow(townIndex+1);townSchedule();});$('town-pause').addEventListener('click',()=>{townPlaying=!townPlaying;townSchedule();});
townPanel.addEventListener('mouseenter',()=>{townHover=true;townSchedule();});townPanel.addEventListener('mouseleave',()=>{townHover=false;townSchedule();});townPanel.addEventListener('focusin',()=>{townFocus=true;townSchedule();});townPanel.addEventListener('focusout',e=>{townFocus=townPanel.contains(e.relatedTarget);townSchedule();});new IntersectionObserver(entries=>{townVisible=entries[0].isIntersecting;townSchedule();}).observe(townPanel);document.addEventListener('visibilitychange',townSchedule);reduced.addEventListener('change',()=>{townPlaying=!reduced.matches;townSchedule();});townSchedule();
